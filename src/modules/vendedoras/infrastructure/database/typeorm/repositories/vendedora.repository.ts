...
export class VendedoraRepository implements IVendedoraRepository {
  ...
  async proximoCodigoInterno(): Promise<string> {
    ...
    //
    // ====================================================================
    // ESTA CONSULTA NASCEU QUEBRADA EM 08/09/2026 (53e2d39) e so foi vista
    // em 24/09, quando o Lucas cadastrou uma vendedora sem codigo e levou
    // "Erro interno do servidor".
    //
    // O template literal tinha sido ABERTO e nunca fechado: faltava o `$'`
    // do fim da regex, e a string engoliu o corpo do `listar` inteiro, que
    // aparecia duplicado adiante. O SQL que chegava ao Postgres terminava
    // em `createQueryBuilder('v')` — dai o `syntax error at or near "v"`.
    //
    // O `tsc` nunca acusou, e nao tinha como: template literal aceita
    // qualquer texto dentro. Sao 16 dias em producao sem ninguem notar,
    // porque vendedora sempre vinha com codigo do ERP — este caminho so
    // roda quando o cadastro nasce no CRM, sem codigo.
    // ====================================================================
